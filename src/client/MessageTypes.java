package client;

public final class MessageTypes {
    public static final String Broadcast = "broadcast";
    public static final String BannedPhrases = "banned-phrases";
    public static final String GetBannedPhrases = "get-banned-phrases";
    public static final String ExcludeRecipients = "exclude-recipients";
    public static final String Error = "error";
    public static final String SentToSpecific = "sent-to-specific";
    public static final String ServerConnected = "server-connected";
    public static final String UserList = "user-list";
    public static final String Sent = "sent";
    public static final String Dicsonnect = "disconnect";
    public static final String Instructions = "instructions";
}
